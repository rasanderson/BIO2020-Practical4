...
Once you have created the plots, try storing them in `lichen_plt3` and `lichen_plt4` objects, and display them side-by-side using `multi_plot()` (requires `bio2020` package; do not worry if not installed):

```{r, echo=FALSE}
p3 <- gf_violin(calcium ~lichen_fct, data = lichen_dat, alpha = 0.3, fill = ~lichen_fct) %>% 
  gf_refine(coord_flip())  %>% 
  gf_sina() %>% 
  gf_labs(x = "Lichen incidence", y = "Calcium", title = "Lichen and Calcium") %>% 
  gf_theme(theme_classic()) %>% 
  gf_theme(legend.position="none") 
...
p4 <- gf_violin(airpoll~lichen_fct, data = lichen_dat, alpha = 0.3, fill = ~lichen_fct) %>% 
  gf_refine(coord_flip())  %>% 
  gf_sina() %>% 
  gf_labs(x = "Lichen incidence", y = "Air Pollution", title = "Lichen and Pollution") %>% 
  gf_theme(theme_classic()) %>% 
  gf_theme(legend.position = "none")
